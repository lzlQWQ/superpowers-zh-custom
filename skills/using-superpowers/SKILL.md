...
如果 CLAUDE.md、GEMINI.md 或 AGENTS.md 对测试策略有明确要求，而某个技能要求更严格或更宽松的测试方式，遵循用户的指令。用户拥有控制权。
...
**刚性的**（调试、完成前验证）：严格遵循。不要偏离纪律。

**策略型的**（testing-policy、TDD）：先按风险选择测试/验证等级。TDD 只在 testing-policy 判定需要时刚性执行。
