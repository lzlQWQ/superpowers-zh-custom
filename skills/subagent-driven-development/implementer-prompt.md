...
Task tool (general-purpose):
  ...
  prompt: |
    ...
    **现在就问。** 在开始工作之前提出任何疑虑。
    ...
    当你确认需求清晰后：
    1. 严格按照任务指定的内容实现
    2. 使用 superpowers:testing-policy 选择测试策略（A/B/C/D）
    3. 按策略编写测试、合并验证或记录跳过自动化测试的理由
    4. 验证实现是否正常工作，或说明已并入哪个验证检查点
    5. 提交你的工作
    6. 自审（见下文）
    7. 汇报
    ...
    **测试：**
    ...
    - 我选择的测试策略是否与风险匹配？
    - 如果要求了 TDD，红灯是否执行到断言层，而不是停在编译错误？
    - 如果合并验证或跳过自动化测试，理由是否清楚且可审查？
    ...
    - 你测试了什么以及测试结果；如果尚未运行，说明将由哪个检查点覆盖
    - 修改了哪些文件
    - 自审发现（如果有）
    - 任何问题或疑虑
