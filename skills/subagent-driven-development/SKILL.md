...
通过为每个任务分派一个全新的子智能体来执行计划。每个任务记录验证预算、测试策略和验证时机，并进行两阶段审查：先审查规格合规性，再审查代码质量。

**为什么用子智能体：** 你将任务委派给具有隔离上下文的专用智能体。通过精心设计它们的指令和上下文，确保它们专注并成功完成任务。它们不应继承你的会话上下文或历史记录——你要精确构造它们所需的一切。这样也能为你自己保留用于协调工作的上下文。

**核心原则：** 每个任务一个全新子智能体 + 验证预算先行 + 按检查点合并验证 + 两阶段审查（先规格后质量）= 高质量、快速迭代
...
- 同一会话（无上下文切换）
- 每个任务全新子智能体（无上下文污染）
- 每个任务记录验证预算和测试策略；低风险任务可并入验证检查点，高风险任务即时验证
...
digraph process {
    ...
    subgraph cluster_per_task {
        ...
        "实现子智能体实现、记录验证预算、按检查点验证、提交、自审" [shape=box];
    ...
    "实现子智能体有疑问?" -> "实现子智能体实现、记录验证预算、按检查点验证、提交、自审" [label="否"];
    "实现子智能体实现、记录验证预算、按检查点验证、提交、自审" -> "分派规格审查子智能体 (./spec-reviewer-prompt.md)";
...
**子智能体应使用：**
- **superpowers:testing-policy** - 每个任务先记录验证预算、测试策略、理由和验证检查点
