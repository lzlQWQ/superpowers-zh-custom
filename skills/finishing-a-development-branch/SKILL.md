...
description: 当实现完成、验证预算已核对、需要决定如何集成工作时使用——通过提供合并、PR 或清理等结构化选项来引导开发工作的收尾
version: "1.0.0"
license: MIT
metadata:
  hermes:
...
**在展示选项之前，按 `superpowers:testing-policy` 和计划检查点核对证据：**

```bash
# 只运行预算内计划要求的验证命令，例如：
npm test / npm run typecheck / cargo test / pytest / go test ./...
```

**如果验证失败：**

```
验证失败（<N> 个失败）。必须先修复才能继续：

[显示失败信息]

在预算内验证证据通过或明确交由用户验收之前无法进行合并/PR。
```

停止。不要继续到步骤 2。

**如果未运行重型验证但策略允许交付用户验收：** 在后续 PR/报告中写清楚“未运行编译/测试；建议用户按以下路径验收”。

**如果预算内验证通过或已明确交付用户验收：** 继续步骤 2。
