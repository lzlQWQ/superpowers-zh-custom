...
1. **确认复现证据或根因证据**
   - 写下最简复现路径、日志、调用链或数据流证据
   - 自动化测试是否编写由 `superpowers:testing-policy` 决定
   - 普通业务 bug 默认实现优先，避免在修复前消耗重型编译/测试预算
   - 只有策略 A 时才使用 `superpowers:test-driven-development`
...
3. **验证修复**
   - 按 `testing-policy` 的验证预算执行轻量核查、目标验证或交付用户验收
   - 如果没有运行重型验证，明确说明未运行编译/测试
   - 给用户可执行的验收路径
...
| "不说明验证范围就交付" | 可以交付用户验收，但必须说明未跑哪些重型验证以及如何验收。 |
...
| **4. 实施** | 确认证据、修复、按预算验证 | 代码改动完成，验证范围和用户验收路径清楚 |
...
**相关技能：**
- **superpowers:testing-policy** - 用于选择验证预算和修复前后的验证等级
- **superpowers:test-driven-development** - 仅在策略 A 时使用
- **superpowers:verification-before-completion** - 在汇报前对齐结论和实际验证范围
